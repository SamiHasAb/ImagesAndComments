		...
		<tr>
			...
			<%
				...
				for (Files file : files) {
					out.print("<tr><td> <img src='"+path+file.getFileName()+"' height='300'> </td>");
					out.print("<td><ul>" 
					...
					out.print("<td><ul>"
					                +"<li><a href='" + request.getContextPath() + "/FilesHandler?action=viewImage&fileId="
							                         + file.getId() + "'>View</a></li>"

					                +"<li><a href='" + request.getContextPath() + "/FilesHandler?action=deleteImage&fileId="
							                         + file.getId()
							                + "' onclick=\"if(!confirm('Are you sure to delete the user?')) return false\">Delete</a></li>"
							+"</ul></td>");
